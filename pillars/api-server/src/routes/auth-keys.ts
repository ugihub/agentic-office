...
const StoreProviderKeySchema = z
  .object({
    provider: z.enum(['anthropic', 'google', 'openai', 'deepseek', 'mistral', 'qwen']),
...
export const authKeyRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.post('/auth/keys', async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  fastify.get('/auth/keys', async (request: FastifyRequest, reply: FastifyReply) => {
    const auth = requireAuth(request, reply)
    ...
    const keys = await ApiKeyModel.find({
      ...
      .select('keyId keyPrefix name status permissions rateLimit usage createdAt lastUsedAt expiresAt')
      .sort({ createdAt: -1 })
      .lean()
      .exec()
  ...
  fastify.delete('/auth/keys/:keyId', async (request: FastifyRequest, reply: FastifyReply) => {
    const auth = requireAuth(request, reply)
    requirePermission(auth, 'keys:write', reply)
  ...
  fastify.post('/auth/provider-keys', async (request: FastifyRequest, reply: FastifyReply) => {
    const auth = requireAuth(request, reply)
  ...
  fastify.delete(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      if (result === null) {
        return reply.status(404).send({ error: 'PROVIDER_KEY_NOT_FOUND', provider })
      }
