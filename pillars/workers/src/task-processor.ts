...
import { recordLlmInvocation, getTaskCostSummary } from '@bureau/cost-analytics'
...
// LLM providers
import { ProviderRegistry, ClaudeProvider, GeminiProvider } from '@bureau/llm-providers'
...
async function releaseUnusedBudgetAfterCostSummary(params: {
  ...
  if (!releaseResult.ok) {
    params.log.error({ err: releaseResult.error.message }, 'Failed to release unused budget')
  }
}
...
async function processTask(job: { data: SubmitJob; attemptsMade: number }): Promise<void> {
  ...
  // ── 1. Load task ─────────────────────────────────────────────────────────────
  const task = await TaskEnvelopeModel.findOne({ taskId, tenantId }).lean().exec()
  if (!task) {
  ...
  if (task.currentStage === 'Formatting') {
    const draft = (task.intermediateOutputs.production as { draft?: string } | null)?.draft
    if (draft === undefined || draft.trim().length === 0) {
      await markFailed(taskId, tenantId, correlationId, 'Cannot complete best-effort task without production draft')
      return
    }
  ...
  if (task.currentStage !== 'Submitted') {
    jobLog.warn({ currentStage: task.currentStage }, 'Task already processing — idempotent skip')
    return
  }
  ...
  await transitionStage(taskId, tenantId, 'Submitted', 'Preparing', 'task-processor', correlationId)
  ...
  const hrResult = selectModelForTask(
    prompt,
    { hasCode: pathSignals.hasCode, hasResearch: pathSignals.hasResearch, tokenCount: tokens },
    executionPath,
    task.originalRequest.constraints.preferredModelTier,
  )

  if (!hrResult.ok) {
    await markFailed(taskId, tenantId, correlationId, `HR SSC failed: ${hrResult.error.message}`)
    return
  }

  const { selectedModel, complexity, escalationChain } = hrResult.value
  jobLog.info(
    { selectedModel, complexityScore: complexity.score, escalationSteps: escalationChain.entries.length },
  ...
  if (!complianceResult.ok) {
    ...
    await markFailed(taskId, tenantId, correlationId, `Compliance blocked task: ${complianceResult.error.message}`)
    return
  }
  ...
  const financeChain: Array<{ attempt: number; model: string; maxCostUsd: string }> =
  ...
  const preApproval = await preApproveEscalationChain(financeDeps, tenantId, financeChain)
  if (!preApproval.ok) {
    await markFailed(taskId, tenantId, correlationId, `Finance pre-approval failed: ${preApproval.error.message}`)
    return
  }
  ...
  if (!reserveResult.ok) {
    jobLog.warn({ err: reserveResult.error.message }, 'Finance SSC: budget insufficient — entering AwaitingUserDecision')
    await TaskEnvelopeModel.updateOne(
      { taskId, tenantId },
      {
        $set: {
  ...
  if (executionPath === 'full') {
    await transitionStage(taskId, tenantId, 'Preparing', 'Researching', 'task-processor', correlationId)
  ...
  for (let attempt = 1; attempt <= Math.min(escalationEntries.length, 3); attempt++) {
    ...
    jobLog.info({ attempt, model, attemptReason }, 'Production attempt starting')

    // Create ChunkWorker with LLM call + cost recording
    const chunkWorker = new ChunkWorker(
      async ({ model: m, prompt: p }) => {
        const start = Date.now()
        const result = await registry.generate(m, { prompt: p, signal: ac.signal })
        ...
        void recordLlmInvocation({
          ...
          provider: result.value.modelUsed.includes('claude') ? 'anthropic' : 'google',
          ...
          escalationTier: attempt === 1 ? 'tier1' : attempt === 2 ? 'tier2' : 'tier3',
          durationMs,
        })
    ...
    if (!productionResult.ok) {
      jobLog.error({ attempt, err: productionResult.error.message }, 'Production attempt failed')
      if (attempt >= Math.min(escalationEntries.length, 3)) {
        await markFailed(taskId, tenantId, correlationId, `Production failed after ${attempt} attempts: ${productionResult.error.message}`)
        return
      }
      continue
    }

    const draft = (productionResult.value.result as { draft?: string }).draft ?? ''
    ...
    jobLog.info({ attempt, draftLength: draft.length }, 'Production attempt succeeded')

    // ── QA Gate ──────────────────────────────────────────────────────────────
    await transitionStage(taskId, tenantId, 'Producing', 'Reviewing', 'task-processor', correlationId)
    ...
    if (!qaResult.ok) {
      ...
      jobLog.warn({ attempt, err: qaResult.error.message }, 'QA max retries exceeded')
    ...
    jobLog.info(
      { attempt, passed: qa.passed, score: qa.overallScore, escalation: qa.escalationRecommended },
      'QA gate result',
    )
    ...
    if (attempt < Math.min(escalationEntries.length, 3)) {
      jobLog.info({ attempt, nextAttempt: attempt + 1 }, 'QA failed — escalating to next model tier')
      await transitionStage(taskId, tenantId, 'Reviewing', 'Producing', 'task-processor', correlationId)
      await TaskEnvelopeModel.updateOne({ taskId, tenantId }, { $inc: { 'retryCount.qa': 1 } }).exec()
      continue
    }

    // All escalation attempts exhausted — AwaitingUserDecision with best_effort
    const bestEffortDraft = draft
    jobLog.warn({}, 'QA failed all escalation attempts — entering AwaitingUserDecision')
  ...
  if (qaPassedAt === -1 && qaOutput === null) {
    await markFailed(taskId, tenantId, correlationId, 'Production loop completed without QA result')
    return
  }

  // ── 11. Marketing pipeline ────────────────────────────────────────────────────
  await transitionStage(taskId, tenantId, 'Reviewing', 'Formatting', 'task-processor', correlationId)
  ...
  if (!marketingResult.ok) {
    jobLog.error({ err: marketingResult.error.message }, 'Marketing pipeline failed')
    await markFailed(taskId, tenantId, correlationId, `Marketing failed: ${marketingResult.error.message}`)
    return
  }
  ...
  if (!releaseResult.ok) {
    jobLog.error({ err: releaseResult.error.message }, 'Failed to release unused budget')
  }
}
