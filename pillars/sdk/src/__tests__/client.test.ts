...
describe("BureauClient", () => {
  ...
  describe("submitTask()", () => {
    ...
    it("throws BureauError on 4xx", async () => {
      ...
      await expect(client.submitTask({ prompt: "test" })).rejects.toThrow(
        BureauError,
      );
    });
  ...
  describe("listTasks()", () => {
    it("unwraps paginated server response", async () => {
      mockResponse({
        tasks: [{ taskId: "task_001", currentStage: "Completed" }],
      });
  ...
  describe("listApiKeys()", () => {
    ...
    it("unwraps server keys response", async () => {
      mockResponse({
        keys: [{ keyId: "key_001", name: "test", keyPrefix: "bureau_live" }],
      });
  ...
  describe("createApiKey()", () => {
    it("POSTs to /api/v1/auth/keys", async () => {
      mockResponse({
        plaintext: "bureau_live_xxx",
        keyId: "key_001",
        name: "test",
        keyPrefix: "bureau_live",
        permissions: [],
      });
  ...
  describe("waitForTask()", () => {
    ...
    it("throws on Failed state", async () => {
      ...
      await expect(
        client.waitForTask("task_001", { intervalMs: 1 }),
      ).rejects.toThrow(BureauError);
    });
