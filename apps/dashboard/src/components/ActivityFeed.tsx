...
const STAGE_EVENT: Partial<Record<TaskStage, { icon: string; iconCls: string; verb: string }>> = {
  Completed:            { icon: "✓", iconCls: "bg-emerald-900/40 text-emerald-400", verb: "completed" },
  Failed:               { icon: "✗", iconCls: "bg-red-900/40 text-red-400",         verb: "failed" },
  AwaitingUserDecision: { icon: "⚠", iconCls: "bg-amber-900/40 text-amber-400",    verb: "needs decision" },
  Producing:            { icon: "⚙", iconCls: "bg-violet-900/40 text-violet-400",  verb: "producing" },
  Submitted:            { icon: "▶", iconCls: "bg-blue-900/40 text-blue-400",       verb: "submitted" },
};
...
export function ActivityFeed({ tasks }: ActivityFeedProps) {
  ...
  if (items.length === 0) {
    return <p className="py-4 text-center text-sm text-muted">No recent activity.</p>;
  }
        ...
        <li key={item.id} className="flex items-start gap-3 py-3 first:pt-0 last:pb-0">
          ...
          <div className="min-w-0">
            <p className="text-xs text-secondary leading-relaxed">
              <strong className="font-medium text-primary/90">Task {item.verb}</strong>
              {" — "}
              {item.preview}
            </p>
