...
export function MetricsRow({ tasks }: MetricsRowProps) {
  ...
  const awaiting = tasks.filter(
    (t) => t.currentStage === "AwaitingUserDecision",
  ).length;
  const successRate =
    completed + failed > 0
    ...
    <div className="grid grid-cols-4 gap-3">
      {metrics.map((m) => (
        ...
        >
          ...
          <p
            className={`mt-2 text-3xl font-bold tracking-tight ${m.valueColor}`}
          >
            {m.value}
          </p>
          <span
            className={`mt-2 inline-block rounded-md px-2 py-0.5 text-[10px] font-semibold ${m.chipColor}`}
          >
            {m.chip}
          </span>
        </div>
