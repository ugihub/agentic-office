...
export class FormatterWorker implements IWorkerAgent {
  ...
  async execute(
    ...
    const formatted = formatContent(marketingInput.draft, marketingInput.outputFormat)
...
export class CitationWorker implements IWorkerAgent {
  ...
  async execute(
    ...
    const { content, sources } = input as {
      content: string
      sources?: Array<{ url?: string | undefined; content: string; score: number }> | undefined
    }
  ...
  async execute(
    ...
    if (this.deliverFn) {
      try {
        ...
        console.error({ err: msg, taskId: ctx.taskId }, 'Delivery hook failed (non-fatal)')
      }
    }
...
export class MarketingAgent implements IHeadAgent {
  ...
  async execute(ctx: AgentContext): Promise<Result<HeadAgentOutput, Error>> {
    ...
    if (!citationResult.ok) {
      log.warn({ err: citationResult.error.message }, 'Citation injection failed, using uncited content')
...
function injectCitations(
  ...
  const urlSources = sources.filter((s): s is typeof s & { url: string } => !!s.url)
