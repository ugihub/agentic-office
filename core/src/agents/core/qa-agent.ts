...
export class CompletenessCheckerWorker implements IWorkerAgent {
  ...
  async execute(
    ...
    if (this.llmCheckFn) {
      const check = await this.llmCheckFn(
        qaInput.originalPrompt,
        qaInput.draft,
      );
      result = {
      ...
      result = heuristicCompletenessCheck(
        qaInput.draft,
        qaInput.originalPrompt,
      );
    }
...
export class RelevanceCheckerWorker implements IWorkerAgent {
  ...
  async execute(
    ...
    if (this.relevanceFn) {
      const check = await this.relevanceFn(
        qaInput.originalPrompt,
        qaInput.draft,
      );
      result = {
        ...
        score: check.score,
        failureReasons: check.offTopicSections.map(
          (s) => `Off-topic section: ${s}`,
        ),
...
export class QaAgent implements IHeadAgent {
  ...
  async execute(ctx: AgentContext): Promise<Result<HeadAgentOutput, Error>> {
    ...
    if (isFastPath) {
      ...
      const validation = schemaResult.value
    ...
    const [schemaResult, completenessResult, relevanceResult] =
      await Promise.all([
    ...
    if (schemaResult.ok) {
      validations.push(
        schemaResult.value.result as unknown as ValidationResult,
      );
      validatorNames.push("SchemaValidator");
    }
    if (completenessResult.ok) {
      validations.push(
        completenessResult.value.result as unknown as ValidationResult,
      );
      validatorNames.push("CompletenessChecker");
    }
    if (relevanceResult.ok) {
      validations.push(
        relevanceResult.value.result as unknown as ValidationResult,
      );
...
function aggregateQaResults(
  ...
  const anyEscalationRecommended = validations.some(
    (v) => v.escalationRecommended,
  );
...
function heuristicCompletenessCheck(
  draft: string,
  prompt: string,
): ValidationResult {
  ...
  const coverage =
    promptWords.length > 0 ? coveredTerms.length / promptWords.length : 1.0;

  const passed = coverage >= 0.6 && draft.length >= 100;
  const failureReasons: string[] = [];

  if (coverage < 0.6) {
    const missingTerms = promptWords
      .filter((w) => !draftLower.includes(w))
      .slice(0, 3);
...
function heuristicRelevanceCheck(
  draft: string,
  prompt: string,
): ValidationResult {
