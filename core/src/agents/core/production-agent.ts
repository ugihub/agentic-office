...
export type AttemptReason = 'initial' | 'qa_escalation' | 'stall_requeue' | 'user_retry'
...
export class ChunkWorker implements IWorkerAgent {
  ...
  constructor(
    ...
    private readonly recordCostFn?: ((record: ChunkOutput) => Promise<void>) | undefined,
  ) {
...
export interface ProductionAgentDeps {
  ...
  onEscalationNeeded?: ((taskId: string, attemptNumber: number) => Promise<void>) | undefined
}
...
export class ProductionAgent implements IHeadAgent {
  ...
  async execute(ctx: AgentContext): Promise<Result<HeadAgentOutput, Error>> {
    ...
    const chunks = chunkPrompt(productionInput.prompt, productionInput.researchSummary)
        ...
        limit(() => {
          if (ctx.signal.aborted) {
            return Promise.resolve(
              err(new Error(`Production cancelled during chunk ${index}`)) as Result<WorkerOutput, Error>,
            )
          }
    ...
    const aggregatedContent = successfulChunks.map((c) => c.generatedContent).join('\n\n')

    const totalTokensIn = successfulChunks.reduce((sum, c) => sum + c.tokensIn, 0)
    const totalTokensOut = successfulChunks.reduce((sum, c) => sum + c.tokensOut, 0)
...
function chunkPrompt(prompt: string, researchSummary?: string | undefined): string[] {
