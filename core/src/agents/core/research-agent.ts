...
export class WebSearchWorker implements IWorkerAgent {
  ...
  async execute(
    ...
    const results = await this.searchFn(
      researchInput.prompt,
      researchInput.maxResults ?? 5,
    );
...
export class EmbeddingWorker implements IWorkerAgent {
  ...
  async execute(
    ...
    const start = Date.now();
    const { prompt, candidates } = input as {
      prompt: string;
      candidates: Array<{ content: string; score: number }>;
    };
...
export class ResearchAgent implements IHeadAgent {
  ...
  async execute(ctx: AgentContext): Promise<Result<HeadAgentOutput, Error>> {
    ...
    const prompt =
      (ctx as AgentContext & { prompt?: string }).prompt ?? ctx.taskId;

    // Phase 1: Web + KB in parallel
    const [webResult, kbResult] = await Promise.all([
      limit(() =>
        this.deps.webSearchWorker.execute(ctx, {
          prompt,
          taskId: ctx.taskId,
          maxResults: 5,
        }),
      ...
      limit(() =>
        this.deps.knowledgeBaseWorker.execute(ctx, {
          prompt,
          taskId: ctx.taskId,
        }),
      ),
    ...
    if (webResult.ok) {
      ...
      candidates.push(
        ...wr.items.map((i) => ({ content: i.content, score: i.score })),
      );
    } else {
      log.warn(
        { err: webResult.error.message },
        "Web search failed, continuing without it",
      );
    }
    ...
    if (kbResult.ok) {
      ...
      candidates.push(
        ...kr.items.map((i) => ({ content: i.content, score: i.score })),
      );
    } else {
      log.warn(
        { err: kbResult.error.message },
        "KB search failed, continuing without it",
      );
    }
    ...
    if (!embeddingResult.ok) {
      log.warn(
        { err: embeddingResult.error.message },
        "Embedding rerank failed, using raw results",
      );
    }
    ...
    const aggregated = aggregateResearchResults(
      webResult.ok
        ? (webResult.value.result as unknown as ResearchResult)
        : null,
      kbResult.ok ? (kbResult.value.result as unknown as ResearchResult) : null,
      embeddingResult.ok
        ? (embeddingResult.value.result as unknown as ResearchResult)
        : null,
    );
...
function aggregateResearchResults(
  ...
  if (kb) {
    allItems.push(
      ...kb.items.map((i) => ({ ...i, source: "knowledge_base" as const })),
    );
  }
