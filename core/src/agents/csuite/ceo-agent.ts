...
export class CeoAgent implements IHeadAgent {
  ...
  async execute(ctx: AgentContext): Promise<Result<HeadAgentOutput, Error>> {
    ...
    log.info({ executionPath: ctx.executionPath }, 'CEO Agent starting task orchestration')
    ...
    try {
      await this.deps.persistTransition(ctx.taskId, 'Submitted', 'Preparing', ctx.correlationId)
    } catch (e) {
    ...
    const nextState = ctx.executionPath === 'fast' ? 'Producing' : 'Researching'
    try {
      await this.deps.persistTransition(ctx.taskId, 'Preparing', nextState, ctx.correlationId)
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e)
      return err(new Error(`State persistence failed: ${msg}`))
    }
