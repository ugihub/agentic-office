...
export interface EscalationChain {
  ...
  map<T>(
    callbackfn: (
      value: HREscalationEntry,
      index: number,
      array: HREscalationEntry[],
    ) => T,
    thisArg?: unknown,
  ): T[];
}
...
export function calculateComplexityScore(
  ...
  if (
    /hukum|legal|medis|medical|keuangan|financial|engineering|teknik/i.test(
      prompt,
    )
  ) {
    score += 2;
  ...
  const tier: ModelTier =
    score <= 3 ? "economy" : score <= 6 ? "standard" : "premium";
...
export function buildEscalationChain(
  ...
  const usesLegacyComplexityArg =
    arguments.length === 2 && estimatedInputTokens <= 10;
