...
const makeCtx = (executionPath: 'fast' | 'standard' | 'full' = 'standard'): AgentContext & { qaInput?: QaInput } => ({
  taskId: 'task_001',
  tenantId: 'tenant_001',
  userId: 'user_001',
...
describe('SchemaValidatorWorker', () => {
  it('passes valid markdown', async () => {
    ...
    const ctx = makeCtx()
    const result = await worker.execute(ctx, ctx.qaInput as unknown as Record<string, unknown>)
...
describe('QaAgent — fast path', () => {
  it('uses only SchemaValidator on fast path', async () => {
    ...
    expect(result.ok).toBe(true)
    if (result.ok) {
      const output = result.value.result as unknown as { validatorsRun: string[] }
