...
import { reserveBudgetAtomic, releaseBudget } from '../agents/ssc/finance-ssc.js'
import { InsufficientBudgetError } from '@bureau/shared-kernel'
import type { BudgetDocument } from '@bureau/models'

// ─── Mock Budget Model ────────────────────────────────────────────────────────

function createMockBudget(overrides: Partial<BudgetDocument> = {}): BudgetDocument {
  return {
...
function makeMockModel(findOneAndUpdateResult: BudgetDocument | null, findOneResult?: BudgetDocument | null) {
...
describe('Finance SSC — reserveBudgetAtomic', () => {
  ...
  it('succeeds when budget is sufficient', async () => {
    ...
    const result = await reserveBudgetAtomic({ budgetModel: model }, baseRequest)
  ...
  it('uses findOneAndUpdate with $gte condition (atomic)', async () => {
    ...
    const [filter] = (model.findOneAndUpdate as ReturnType<typeof vi.fn>).mock.calls[0] as [Record<string, unknown>]
  ...
  it('returns InsufficientBudgetError when findOneAndUpdate returns null', async () => {
    ...
    const result = await reserveBudgetAtomic({ budgetModel: model }, {
      ...baseRequest,
  ...
  describe('race condition simulation', () => {
    it('only one of two parallel reservations succeeds when budget covers one', async () => {
      ...
      const mockFindOne = vi.fn().mockReturnValue({
        select: vi.fn().mockReturnValue({
          exec: vi.fn().mockResolvedValue(createMockBudget({ isFrozen: false })),
        }),
      })
      ...
      const [result1, result2] = await Promise.all([
        reserveBudgetAtomic({ budgetModel: model }, { ...baseRequest, taskId: 'task_A' }),
        reserveBudgetAtomic({ budgetModel: model }, { ...baseRequest, taskId: 'task_B' }),
      ])
...
describe('Finance SSC — releaseBudget', () => {
  it('releases unconsumed budget back to remaining', async () => {
    ...
    const [, update] = (mockUpdateOne as ReturnType<typeof vi.fn>).mock.calls[0] as [unknown, Record<string, unknown>]
