...
function makeCtx(executionPath: AgentContext['executionPath'] = 'full'): AgentContext {
  return {
    taskId: 'task_esc_001',
...
describe('Escalation chain — QA Agent', () => {
  ...
  it('fast path QA: schema-only, no escalation', async () => {
    ...
    if (result.ok) {
      const output = result.value.result as { passed: boolean; validatorsRun: string[]; escalationRecommended: boolean }
  ...
  it('full path QA: runs 3 validators in parallel', async () => {
    const ctx = makeQaCtx('full', {
      draft: 'A comprehensive detailed analysis of the market trends showing growth across multiple sectors with citations and data points.',
  ...
  it('empty content fails QA with failure reasons', async () => {
    ...
    expect(result.ok).toBe(true)
    if (result.ok) {
      const output = result.value.result as { passed: boolean; failureReasons: string[]; escalationRecommended: boolean }
...
describe('Escalation chain — HR SSC buildEscalationChain', () => {
  ...
  it('totalMaxCostUsd = sum of all entry costs', () => {
    const chain = buildEscalationChain('economy')
    const summed = chain.entries.reduce((sum, e) => sum + parseFloat(e.maxCostUsd), 0)
  ...
  it('economy → standard chain starts cheaper than standard → premium', () => {
    ...
    const standardFirst = parseFloat(fromStandard.entries[0]?.maxCostUsd ?? '0')
