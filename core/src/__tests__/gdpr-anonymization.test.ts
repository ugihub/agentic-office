...
describe("GDPR anonymization", () => {
  ...
  it("sets anonymizedAt in cost_analytics", async () => {
    ...
    const updateArgs = deps.costEventModel.updateMany.mock
  ...
  it("hard-deletes user_provider_keys", async () => {
    ...
    expect(deps.userProviderKeyModel.deleteMany).toHaveBeenCalledWith({
      userId,
    });
  });
  ...
  it("hard-deletes api_keys owned by user", async () => {
    ...
    expect(deps.apiKeyModel.deleteMany).toHaveBeenCalledWith({
      ownerId: userId,
    });
  });
  ...
  it("anonymization is scoped to the specific userId", async () => {
    ...
    const costFilter = deps.costEventModel.updateMany.mock
      .calls[0]?.[0] as Record<string, unknown>;
    expect(costFilter?.["userId"]).toBe("user_specific_001");

    const taskFilter = deps.taskEnvelopeModel.updateMany.mock
      .calls[0]?.[0] as Record<string, unknown>;
