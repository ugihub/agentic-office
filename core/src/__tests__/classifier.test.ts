...
describe('Path Classifier', () => {
  describe('classifyPath()', () => {
    ...
    it('classifies research + long prompt as full', () => {
      const longResearchPrompt = 'Analisis komprehensif ' + 'tren pasar fintech Indonesia '.repeat(20)
    ...
    it('classifies medium research prompt as standard', () => {
      const result = classifyPath({ prompt: 'Bandingkan dua startup AI di Indonesia.' })
      expect(result.path).toBe('standard')
      expect(result.signals.hasResearch).toBe(true)
    })

    it('detects temporal signals', () => {
      const result = classifyPath({ prompt: 'Apa berita terbaru tentang Bitcoin?' })
  ...
  describe('classifyCacheCategory()', () => {
    it('classifies financial prompts', () => {
      expect(classifyCacheCategory('Berapa harga Bitcoin sekarang?')).toBe('financial')
      expect(classifyCacheCategory('Kurs USD/IDR hari ini?')).toBe('financial')
    })

    it('classifies temporal prompts', () => {
      expect(classifyCacheCategory('Apa yang terjadi hari ini?')).toBe('temporal')
    })
    ...
    it('classifies inventory prompts', () => {
      expect(classifyCacheCategory('Apakah produk ini tersedia?')).toBe('inventory')
    })

    it('defaults to default category', () => {
      expect(classifyCacheCategory('Jelaskan konsep machine learning.')).toBe('default')
    })
  })
