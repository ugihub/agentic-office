...
function makeBudgetModel(
  tenantId: string,
  budgetResult: Partial<BudgetDocument> | null,
) {
  const mockFindOneAndUpdate = vi
    .fn()
    .mockImplementation(
      ({ tenantId: filterTenantId }: { tenantId: string }) => ({
...
describe("Tenant isolation", () => {
  describe("Budget isolation", () => {
    it("tenant A reservation only affects tenant A budget", async () => {
      ...
      const filterArg = (
        tenantAModel.findOneAndUpdate as ReturnType<typeof vi.fn>
    ...
    it("budget filter always includes tenantId", async () => {
      ...
      const filterArg = (model.findOneAndUpdate as ReturnType<typeof vi.fn>)
        .mock.calls[0]?.[0] as Record<string, unknown>;
