 ...
 */
export async function encryptProviderKey(plaintext: string): Promise<Result<string, Error>> {
  try {
 ...
 */
export async function decryptProviderKey(encrypted: string): Promise<Result<string, Error>> {
  try {
    const encryptionKeyHex = process.env['API_KEY_ENCRYPTION_KEY']
    if (!encryptionKeyHex) {
      return err(new Error('API_KEY_ENCRYPTION_KEY not set'))
