...
describe('SemanticCache', () => {
  ...
  describe('get()', () => {
    ...
    it('returns null when score below threshold (< 0.95)', async () => {
      const metadata = { ...makeResponse(), cachedAt: Date.now() }
      const client = makeClient({ queryScore: 0.80, queryMetadata: metadata })
      const cache = new SemanticCache(client, mockEmbedFn)
    ...
    it('returns null when score below 0.90 even with custom threshold of 0.80', async () => {
      ...
      const result = await cache.get('claude-haiku-4-5', 'Test', { similarityThreshold: 0.80 })
    ...
    it('returns null when bypass option is set', async () => {
      ...
      const result = await cache.get('claude-haiku-4-5', 'What is TypeScript?', { bypass: true })
      expect(result).toBeNull()
      expect(mockEmbedFn).not.toHaveBeenCalled()
    })
  ...
  describe('set()', () => {
    it('stores response in Upstash with embedding', async () => {
      ...
      await cache.set('claude-haiku-4-5', 'What is TypeScript?', makeResponse())
    ...
    it('SKIPS financial prompts — never upserts', async () => {
      ...
      await cache.set('claude-haiku-4-5', 'harga bitcoin saat ini', makeResponse())
    ...
    it('does not throw when Upstash set fails (non-fatal)', async () => {
      ...
      // Should not throw
      await expect(cache.set('claude-haiku-4-5', 'Hello world', makeResponse())).resolves.toBeUndefined()
    })
  })
  ...
  describe('invalidate()', () => {
    ...
    it('does not throw when delete fails (non-fatal)', async () => {
      ...
      await expect(cache.invalidate('claude-haiku-4-5', 'test')).resolves.toBeUndefined()
    })
  })
})
