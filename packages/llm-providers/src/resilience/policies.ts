...
function isRetryableError(err: unknown): boolean {
  ...
  if (msg.includes("rate limit") || msg.includes("too many requests"))
    return true;
  if (msg.includes("service unavailable") || msg.includes("server error"))
    return true;
...
export function createLlmPolicy(
  ...
  if (!breaker) {
    breaker = circuitBreaker(handleType(Error), {
  ...
  if (!bh) {
    ...
    bh.onReject(() => {
      log.warn(
        { provider, maxConcurrent, maxQueue },
        "Bulkhead rejected — LLM provider at capacity",
      );
    });
...
export function getCircuitBreakerState(
  ...
  const state = (breaker as unknown as { state?: { value?: string } }).state
    ?.value;
