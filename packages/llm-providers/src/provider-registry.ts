...
export class ProviderRegistry {
  ...
  async generate(
    ...
    try {
      ...
      log.error(
        { modelId, provider: providerName, err: error.message },
  ...
  async generateWithFallback(
    ...
    Result<
      GenerateResult & { usedFallback: boolean; originalModel: string },
      Error
    >
  > {
    ...
    log.warn(
      {
        modelId,
        err: primaryResult.error.message,
        fallbackCount: fallbackModelIds.length,
      },
    ...
    for (const fallbackId of fallbackModelIds) {
      ...
      if (fallbackResult.ok) {
        log.info(
          { originalModel: modelId, usedModel: fallbackId },
