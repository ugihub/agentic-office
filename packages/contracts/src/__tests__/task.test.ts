...
import { CreateTaskRequestSchema, TaskDecisionRequestSchema, TaskFeedbackRequestSchema } from '../task.js'
...
describe('Task contracts', () => {
  describe('CreateTaskRequestSchema', () => {
    ...
    it('rejects prompt over 50000 chars', () => {
      const result = CreateTaskRequestSchema.safeParse({ prompt: 'a'.repeat(50001) })
      expect(result.success).toBe(false)
    })
  ...
  describe('TaskDecisionRequestSchema', () => {
    it('accepts best_effort', () => {
      const result = TaskDecisionRequestSchema.safeParse({ action: 'best_effort' })
      expect(result.success).toBe(true)
    })

    it('accepts add_budget', () => {
      const result = TaskDecisionRequestSchema.safeParse({ action: 'add_budget' })
      expect(result.success).toBe(true)
    })
