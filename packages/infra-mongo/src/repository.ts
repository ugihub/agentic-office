 ...
 */
import type { Document, FilterQuery, Model, UpdateQuery, QueryOptions } from 'mongoose'
...
export class BaseRepository<TDoc extends Document & { tenantId: string }> {
  ...
  async count(filter: FilterQuery<TDoc>, tenantId: string): Promise<Result<number, Error>> {
    return tryAsync(async () =>
  ...
  /** Check existence */
  async exists(filter: FilterQuery<TDoc>, tenantId: string): Promise<Result<boolean, Error>> {
    const result = await tryAsync(async () =>
