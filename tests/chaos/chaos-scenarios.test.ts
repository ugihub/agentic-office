...
import { classifyCacheCategory, SYSTEM_FLOOR_TTL } from '../../packages/llm-providers/src/cache/category-cache.js'
import { SemanticCache } from '../../packages/llm-providers/src/cache/upstash-vector-cache.js'

// ─── Chaos scenario helpers ───────────────────────────────────────────────────

function makeFlakeyFn<T>(successAfterN: number, failWith: Error, successValue: T) {
  let callCount = 0
  return vi.fn(async () => {
    callCount++
...
describe('Chaos: Redis unavailability', () => {
  ...
  it('CHAOS-1b: Semantic cache get returns null when Upstash is down', async () => {
    const downClient = {
      upsert: vi.fn().mockRejectedValue(new Error('Upstash: connection refused')),
      query: vi.fn().mockRejectedValue(new Error('Upstash: connection refused')),
      delete: vi.fn().mockRejectedValue(new Error('Upstash: connection refused')),
    }
  ...
  it('CHAOS-1c: Semantic cache set does not throw when Upstash is down', async () => {
    ...
    await expect(cache.set('claude-sonnet-4-6', 'Test prompt', {
      text: 'Response',
...
describe('Chaos: Concurrent budget depletion simulation', () => {
  it('CHAOS-4a: Simulated atomic budget reservation — only one of two concurrent reservers wins', async () => {
    ...
    let budgetRemaining = 0.10 // $0.10 total budget

    const atomicReserve = async (taskId: string, amount: number): Promise<{ ok: boolean }> => {
  ...
  it('CHAOS-4b: Budget exhaustion with 50 concurrent workers — balance stays >= 0', async () => {
    let budget = 5.00 // $5 total
    const COST_PER_TASK = 0.50 // $0.50 each → max 10 tasks succeed
...
describe('Chaos: Result<T,E> error propagation', () => {
  ...
  it('CHAOS-5c: Chaining err does not throw even after multiple failures', () => {
    ...
    const step2 = step1.ok ? ok('step2') : err(new Error('Step 2 skipped due to step 1 failure'))
...
describe('Chaos: Flaky dependency retry pattern', () => {
  it('CHAOS-6a: Function succeeds after N transient failures', async () => {
    ...
    const flakeyFn = makeFlakeyFn(2, new Error('Service unavailable'), 'success')
  ...
  it('CHAOS-6b: Function fails permanently after max retries', async () => {
    const alwaysFails = vi.fn().mockRejectedValue(new Error('Permanent failure'))
