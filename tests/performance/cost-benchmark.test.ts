...
describe('Cost Benchmark — Smart Routing Saves >= 60%', () => {
  ...
  describe('Baseline: always use most expensive model', () => {
    it('calculates baseline (Opus) cost', () => {
      const baselineCost = estimateCost('claude-opus-4-6', TOKENS_IN, TOKENS_OUT)
      ...
      expect(baselineCost!).toBeCloseTo(0.10, 2)
    })
  })
  ...
  describe('Smart routing: uses cheapest model first', () => {
    it('economy model (Haiku) costs 60%+ less than Opus for same tokens', () => {
      ...
      const haikuCost = estimateCost('claude-haiku-4-5-20251001', TOKENS_IN, TOKENS_OUT)!

      const savings = (opusCost - haikuCost) / opusCost
      console.log(`Opus: $${opusCost.toFixed(4)} | Haiku: $${haikuCost.toFixed(4)} | Savings: ${(savings * 100).toFixed(1)}%`)

      // Haiku is significantly cheaper than Opus
      expect(savings).toBeGreaterThanOrEqual(0.60) // >= 60% savings
    })

    it('DeepSeek V3.2 saves even more vs Opus', () => {
      const opusCost = estimateCost('claude-opus-4-6', TOKENS_IN, TOKENS_OUT)!
      const deepseekCost = estimateCost('deepseek-v3-2', TOKENS_IN, TOKENS_OUT)!

      const savings = (opusCost - deepseekCost) / opusCost
      console.log(`Opus: $${opusCost.toFixed(4)} | DeepSeek: $${deepseekCost.toFixed(4)} | Savings: ${(savings * 100).toFixed(1)}%`)

      expect(savings).toBeGreaterThanOrEqual(0.60)
    })
  })
  ...
  describe('Escalation chain cost vs naive', () => {
    it('task completing on Attempt 1 (economy) vs always Opus', () => {
      ...
      console.log(`  Smart Attempt 1 (${attempt1Model}): $${smartCostAttempt1.toFixed(4)}`)
    ...
    it('even worst-case (all 3 attempts) still saves vs 3x Opus', () => {
      ...
      expect(savings).toBeGreaterThanOrEqual(0.40) // At least 40% even in worst case
    })
  })
  ...
  describe('Prompt caching savings (cachedTokens)', () => {
    it('cached tokens reduce cost significantly', () => {
      ...
      // Without caching
      const costWithoutCache = estimateCost('claude-sonnet-4-6', TOTAL_INPUT, OUTPUT)!

      // With caching: cachedTokens at ~10% of normal input price (via cachedInputPer1M)
      const costWithCache = estimateCost('claude-sonnet-4-6', TOTAL_INPUT, OUTPUT, CACHED)!

      const cacheSavings = (costWithoutCache - costWithCache) / costWithoutCache
      ...
      expect(cacheSavings).toBeGreaterThan(0.30)
    })
  })
})
...
describe('Pricing Config Validation', () => {
  it('all registered models have valid pricing', () => {
    for (const pricing of MODEL_PRICING) {
      expect(pricing.inputPer1M, `${pricing.modelId}: inputPer1M <= 0`).toBeGreaterThan(0)
      expect(pricing.outputPer1M, `${pricing.modelId}: outputPer1M <= 0`).toBeGreaterThan(0)
      expect(pricing.tier, `${pricing.modelId}: tier missing`).toMatch(/^(economy|standard|premium)$/)
    }
  })

  it('premium models are more expensive than economy', () => {
    const economyInputPrices = MODEL_PRICING
      .filter((p) => p.tier === 'economy')
      .map((p) => p.inputPer1M)

    const premiumInputPrices = MODEL_PRICING
  ...
  it('cost deviation alert threshold is 20%', () => {
    expect(COST_DEVIATION_ALERT_THRESHOLD).toBe(0.20)
  })
...
describe('Fast Path vs Full Path Cost Comparison', () => {
  it('fast path uses fewer LLM calls (lower cost)', () => {
    ...
    expect(savings).toBeGreaterThan(0.50) // 50%+ savings from fewer calls
  })
...
describe('Escalation Chain Structure', () => {
  ...
  it('totalMaxCostUsd is sum of all entries', () => {
    const chain = buildEscalationChain('economy')
    const sumEntries = chain.entries.reduce((s, e) => s + parseFloat(e.maxCostUsd), 0)
