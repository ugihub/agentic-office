...
} from "../../core/src/agents/ssc/hr-ssc.js";
...
function benchmark(name: string, fn: () => unknown, iterations = 1000): number {
  ...
  console.log(
    `[Benchmark] ${name}: avg ${avgMs.toFixed(3)}ms over ${iterations} iterations`,
  );
  return avgMs;
}
...
describe("Latency: Path Classifier (must be < 1ms avg)", () => {
  it("classifyPath runs in < 1ms average", () => {
    const avg = benchmark("classifyPath", () =>
      classifyPath({
        prompt: "Explain renewable energy trends in Southeast Asia.",
      }),
    );
  ...
  it("100 concurrent classifyPath calls complete quickly", async () => {
    ...
    await Promise.all(
      Array.from({ length: 100 }, (_, i) =>
        Promise.resolve(
          classifyPath({ prompt: `Test prompt ${i} for classification.` }),
        ),
      ),
    );
...
describe("SLO Reference Values", () => {
  ...
  it("cost benchmark target: smart routing >= 60% savings", () => {
    const SAVINGS_TARGET = 0.6;
    expect(SAVINGS_TARGET).toBe(0.6);
  });
});
