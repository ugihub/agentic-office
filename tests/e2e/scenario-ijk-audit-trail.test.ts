...
import {
  ...
  SYSTEM_FLOOR_TTL,
  TENANT_MAX_TTL,
} from "../../core/src/path-classifier/classifier.js";
...
describe("Scenario I — Fast Path: Simple Prompt → 3 Divisions Only", () => {
  ...
  it("I4: Fast path includes Finance (budget check cannot be skipped)", () => {
    ...
    const productionIndex = plan.stages.findIndex(
      (s) => s.division === "Production",
    );
...
describe("Scenario J — Spending Anomaly Detection", () => {
  ...
  it("J3: Per-tenant baseline, not global", () => {
    ...
    const tenantAAvg = 50.0;
    const tenantBAvg = 2.0;
    const todayBoth = 50.0;
  ...
  it("J4: Alert payload has required fields", () => {
    const alertPayload = {
      ...
      currentHourCostUsd: 35.0,
      rollingAvgHourlyCostUsd: 10.0,
      multiplier: 3.5,
  ...
  it("J5: 100% quota triggers all-task freeze", () => {
    const quota = { totalUsd: 100.0, consumed: 100.0, isFrozen: false };
  ...
  describe("K3: Category-based TTL bounds", () => {
    it("all category floor TTLs are non-negative", () => {
      for (const [category, ttl] of Object.entries(SYSTEM_FLOOR_TTL)) {
        expect(
          ttl,
          `Floor TTL for ${category} is negative`,
        ).toBeGreaterThanOrEqual(0);
      }
    });

    it("all category max TTLs >= floor TTLs", () => {
      for (const category of Object.keys(SYSTEM_FLOOR_TTL) as Array<
        keyof typeof SYSTEM_FLOOR_TTL
      >) {
        const floor = SYSTEM_FLOOR_TTL[category];
        const max = TENANT_MAX_TTL[category];
        expect(max, `Max TTL for ${category} < floor`).toBeGreaterThanOrEqual(
          floor,
        );
      }
    });
  ...
  describe("AT3: Audit entry required fields", () => {
    it("all required fields present in audit entry", () => {
      ...
      for (const field of requiredFields) {
        expect(
          auditEntry[field as keyof typeof auditEntry],
          `Missing field: ${field}`,
        ).toBeTruthy();
      }
    });
